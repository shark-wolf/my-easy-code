...
class PreviewTemplateAction : AnAction(), DumbAware {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val defPath = expand(defOut, baseDir, pkg, sampleTable(project))
        ...
        val finalDir = if (!overrideRaw.isNullOrBlank()) {
            val p = expand(overrideRaw!!, baseDir, pkg, sampleTable(project))
            val ov = Path.of(p)
        ...
        val dtoPkg = packageFromDir(finalDir, baseDir, pkg)
        val voPkg = dtoPkg
        val entityPkg = dtoPkg
        val servicePkg = dtoPkg
        val mapperPkg = dtoPkg
        val controllerPkg = dtoPkg
        ...
        val ft = com.intellij.openapi.fileTypes.FileTypeManager.getInstance().getFileTypeByFileName(defFile)
        val lvf = LightVirtualFile(defFile, ft, out.toString())
